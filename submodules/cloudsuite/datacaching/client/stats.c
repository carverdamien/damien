...
void printGlobalStats(struct config* config) {
  ...
  struct timeval currentTime;
  gettimeofday(&currentTime, NULL);
  double t = currentTime.tv_sec + 1e-6*currentTime.tv_usec;
  ...
  for(i=0; i<config->n_workers; i++) {
    ...
    printf(
	 ...
	 global_stats.hits,
	 global_stats.misses,
	 1000*getAvg(&global_stats.response_time),
	 1000*q90,
	 1000*q95,
	 1000*q99,
	 1000*std,
	 1000*global_stats.response_time.min,
	 1000*global_stats.response_time.max,
