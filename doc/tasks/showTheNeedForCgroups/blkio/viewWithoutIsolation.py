def view(runId):
    ...
    x = rate(profiles[0], runId, flowop, key)
    annotation = '<a href="http://indium.rsr.lip6.fr/run/%s">X</a>' %  (runId)
    yield {'x':x, 'y':perfA, 'label':'.'.join(['procA.mean(%s in %s)' % (flowop,key)]), 'annotation':annotation}
    yield {'x':x, 'y':perfB, 'label':'.'.join(['procB.mean(%s in %s)' % (flowop,key)]), 'annotation':annotation}
